...
- `shape` overrides the default rectangle: `pill` (rounded), `square` (1:1, no padding), `circle` (round, 1:1). An icon-only `square` or `circle` button needs an accessible name: a string `tooltip` supplies one, otherwise pass `aria-label`.
- `loading` swaps the content for a spinner while keeping the button's footprint stable.
- `tooltip` wraps the trigger in a Tooltip — no need to compose it manually. The tooltip is visual only; it names the button only when the button is `square` or `circle` and has no `aria-label` or `aria-labelledby` of its own.
