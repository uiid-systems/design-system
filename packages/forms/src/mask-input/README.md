...
Everything [`Input`](../input/README.md) offers is here too: `label`, `description`, `before`/`after` slots, `size`, `ghost`, `fullwidth`, and the `required`/`disabled`/`readOnly` states.
