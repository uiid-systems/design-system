import {
  MASK_INPUT_REGEX,
  MASK_INPUT_PAST_YEARS_LIMIT,
  MASK_INPUT_FUTURE_YEARS_LIMIT,
  MASK_INPUT_DAYS_IN_MONTH,
...
export const datePattern: MaskPattern = {
  ...
  validate: (value) => {
    ...
    const minYear = currentYear - MASK_INPUT_PAST_YEARS_LIMIT;
    const maxYear = currentYear + MASK_INPUT_FUTURE_YEARS_LIMIT;
    ...
    const maxDays =
      month === 2 && ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0)
        ? 29
        : (MASK_INPUT_DAYS_IN_MONTH[month - 1] ?? 31);
